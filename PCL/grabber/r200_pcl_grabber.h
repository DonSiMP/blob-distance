...
namespace pcl
{
	...
	class R200Grabber : public pcl::Grabber
	{
	public:
		R200Grabber();
		virtual ~R200Grabber() throw ();
		virtual void start();
		virtual void stop();
	...
	pcl::R200Grabber::~R200Grabber() throw()
	{
		stop();
	...
	void pcl::R200Grabber::start()
	{

		running = true;

		thread = boost::thread(&R200Grabber::threadFunction, this);
	}

	void pcl::R200Grabber::stop()
	{
	...
	bool pcl::R200Grabber::isRunning() const
	{
		boost::unique_lock<boost::mutex> lock(mutex);
	...
	std::string pcl::R200Grabber::getName() const
	{
		return std::string("R200Grabber");
	}

	float pcl::R200Grabber::getFramesPerSecond() const
	{
		return FPS;
	}

	void pcl::R200Grabber::threadFunction()
	{
		while (!quit) {
			boost::unique_lock<boost::mutex> lock(mutex);
